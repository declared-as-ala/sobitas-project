...
class SeoProductReviewsSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $seoReviews = [
            // French/Derja Reviews with high SEO impact keywords
            [
                'stars' => 5,
            ...
            [
                'stars' => 5,
                'comment' => "Si vous cherchez de la whey protein tunisie pas cher avec une qualité certifiée par le ministère de la santé, foncez les yeux fermés. Excellent service.",
            ],
            [
                'stars' => 5,
            ...
            [
                'stars' => 4,
                'comment' => "BCAA bien dosé pour la récupération musculaire. Le prix complement alimentaire tunisie is très correct. Merci protein.tn !",
            ],

            // Tunisian Arabic & Classic Arabic Reviews with high SEO impact keywords
            [
                'stars' => 5,
            ...
            [
                'stars' => 5,
                'comment' => "بروتين واي أصلي وممتاز لزيادة الكتلة العضلية بدون دهون. أرخص سعر بروتين في تونس مقابل جودة ممتازة.",
            ],
            [
                'stars' => 4,
        ...
        // 4. Populate the reviews table
        $reviewRecords = [];
        $productCount = $products->count();

        for ($i = 0; $i < 30; $i++) {
            // Distribute reviews evenly among active products
            $product = $products[$i % $productCount];
            $user = $users[$i % $users->count()];
            $reviewData = $seoReviews[$i];

            // Spread out the created_at dates over the last 3 months to make it look organic
            $createdAt = Carbon::now()->subDays(random_int(1, 90))->subHours(random_int(1, 23))->subMinutes(random_int(1, 59));
        ...
        // Insert records
        Review::query()->insert($reviewRecords);

        $this->command?->info("Seeding completed: 30 highly optimized SEO reviews (Tunisian/French/Arabic) successfully inserted!");
    }
}
