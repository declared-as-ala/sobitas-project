...
<head>
    ...
    <style type="text/css">
        ...
        @media only screen and (max-width: 620px) {
            ...
            .product-cell-mobile-block { display: block !important; width: 100% !important; padding: 6px 12px !important; }
        ...
        <tr>
            <td align="center" style="padding: 20px 12px;">
                <table role="presentation" class="email-wrapper" cellpadding="0" cellspacing="0" width="600" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.08), 0 2px 4px -2px rgba(0,0,0,0.06);">
                    <!-- Header -->
                    <tr>
                        <td class="header-padding" style="padding: 36px 32px 28px; text-align: center; background: linear-gradient(135deg, #b91c1c 0%, #dc2626 50%, #c2410c 100%);">
                            <table role="presentation" cellpadding="0" cellspacing="0" width="100%" align="center">
                                <tr>
                                    <td align="center">
                                        <img src="{{ $logoUrl }}" alt="SOBITAS" width="112" height="auto" style="display: block; max-width: 112px; background: #fff; padding: 10px; border-radius: 12px; margin: 0 auto 18px;">
                                    </td>
                                </tr>
                                <tr>
                                    <td align="center" class="headline" style="color: #ffffff; font-size: 26px; font-weight: 700; line-height: 1.25; letter-spacing: -0.02em;">Merci pour votre commande</td>
                                </tr>
                                <tr>
                                    <td align="center" class="subhead" style="color: rgba(255,255,255,0.95); font-size: 15px; padding-top: 10px;">Commande #{{ $commande->numero }} · {{ $dateFormatted }}</td>
                                </tr>
                            </table>
                        </td>
                    </tr>
                    <!-- Order summary card -->
                    <tr>
                        <td class="email-section" style="padding: 24px 32px 20px;">
                                    ...
                                    <td class="card-padding">
                                        <table role="presentation" cellpadding="0" cellspacing="0" width="100%">
                                            <tr>
                                                ...
                                                <td align="right" style="font-size: 22px; font-weight: 700; color: #b91c1c;">{{ number_format($totalTtc, 2, ',', ' ') }} TND</td>
                                            </tr>
                                            @if($commande->payment_method ?? null)
                                            <tr><td colspan="2" style="padding-top: 12px; font-size: 14px; color: #6b7280;">Paiement : {{ $commande->payment_method === 'cod' ? 'À la livraison' : ($commande->payment_method === 'card' ? 'Carte bancaire' : $commande->payment_method) }}</td></tr>
                                            @endif
                                            @if($commande->livraison ?? null)
                                            <tr><td colspan="2" style="font-size: 14px; color: #6b7280;">Livraison : {{ is_numeric($commande->livraison) ? 'Standard' : $commande->livraison }}</td></tr>
                                            @endif
                                        </table>
                                    </td>
                                </tr>
                    ...
                    <tr>
                        <td class="email-section" style="padding: 0 32px 20px;">
                            <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px;">
                                <tr><td style="font-size: 12px; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em; padding-bottom: 10px;">Informations client</td></tr>
                    ...
                    <tr>
                        <td class="email-section" style="padding: 0 32px 28px; text-align: center;">
                            <table role="presentation" cellpadding="0" cellspacing="0" align="center" class="btn-wrap">
                                <tr>
                                    <td align="center" style="border-radius: 10px; background: #b91c1c;">
                                        <a href="{{ $orderUrl }}" target="_blank" class="btn" style="display: inline-block; padding: 16px 32px; font-size: 16px; font-weight: 600; color: #ffffff !important; text-decoration: none;">Suivre ma commande</a>
                                    </td>
                                </tr>
                            </table>
                        </td>
                    ...
                    <tr>
                        <td class="footer-padding" style="padding: 24px 32px; background-color: #111827; color: #9ca3af; font-size: 13px; text-align: center;">
                            <p style="margin: 0 0 6px;">Merci de votre confiance.</p>
                            <p style="margin: 0 0 6px;">Contact : {{ config('mail.reply_to.address', 'contact@sobitas.tn') }}</p>
                            <p style="margin: 14px 0 0; font-size: 11px; color: #6b7280;">SOBITAS — Confirmation de commande</p>
                        </td>
                    </tr>
                </table>
            </td>
