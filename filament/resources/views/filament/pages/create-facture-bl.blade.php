...
.bl-net-row{background:#fff7ed}
.bl-net-row td{font-size:15px!important;font-weight:800!important;color:#9a2c06!important}
...
@media (max-width:640px){
  .bl-wrap{padding:10px}
  .bl-form{padding:14px}
  .bl-top{flex-direction:column;gap:14px}
  .bl-bottom{flex-direction:column;gap:14px}
  .bl-totals{flex:1 1 auto;width:100%}
}
</style>
...
function blWaitAndBoot() {
    if (typeof $ !== 'undefined' && $.fn && $.fn.select2) {
        blBootstrap();
    } else {
        setTimeout(blWaitAndBoot, 80);
    }
}

window.blFormReinit = blWaitAndBoot;

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', blWaitAndBoot);
} else {
    blWaitAndBoot();
}
if (!window._blNavListenerActive) {
    window._blNavListenerActive = true;
    document.addEventListener('livewire:navigated', blWaitAndBoot);
}
...
function blCreateClient() {
    Swal.fire({
        ...
        preConfirm: function () {
            ...
            var matricule = (document.getElementById('bl_sw_mf')?.value || '').trim();
            if (!name) {
