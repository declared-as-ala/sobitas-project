 ...
 *
 * IT PREPARES `affilie_payouts` ROWS AND STOPS. Every other entry in this file changes data that
 * can be edited back; this one would move real money out of a real bank account if it were allowed
 * to, and a cron that pays affiliates while nobody is watching is precisely the hole an affiliate
...
/*
|--------------------------------------------------------------------------
| Product FAQ, derived from the label
|--------------------------------------------------------------------------
| products:faq-from-label reads each product's Supplement Facts panel and title and writes a
| structured FAQ. Nothing is generated by a model: the serving size, the servings per container and
| the pack format are lifted verbatim, and a question whose fact is missing is simply not asked.
|
| It runs NIGHTLY AND UNLATCHED, unlike seo:ctr-pass, and the difference is deliberate. The CTR copy
| is a one-off statement about six pages that a human may later edit. This is a continuous backfill:
| products gain their Supplement Facts panel hours or days after they are promoted, as the content
| import works through the harvest, so the set of products that CAN have an FAQ grows every night.
| The command skips any product whose `faq` column is already populated, so a human edit is safe and
| a second pass over the same product is free.
|
| 300 a night against a catalogue heading for tens of thousands is slow on purpose. This writes to
| the products table, and a bug that reaches every row at once is a restore; a bug that reaches 300
| rows is a morning's work. Raise it once a few nights have been inspected.
*/
Schedule::command('products:faq-from-label --limit=300 --apply')
    ->dailyAt('03:10')
    ->withoutOverlapping(60)
