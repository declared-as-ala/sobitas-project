        ...
        return array_merge(parent::getHeaderActions(), [
            Actions\Action::make('send')
                ...
                ->action(function () {
                    ...
                    if ($email === '') {
                        Notification::make()
                            ->title('Le client n\'a pas d\'adresse email.')
                            ->body('Veuillez renseigner l\'email du client pour envoyer la facture.')
                            ->actions([
                                Notification\Action::make('edit_client')
                                    ->label('Ouvrir le client')
                                    ->url(ClientResource::getUrl('edit', ['record' => $client->id]))
                                    ->openUrlInNewTab(),
                            ])
                            ->danger()
                            ->send();
            ...
            Actions\Action::make('pdf')
                ...
                ->url(route('facture-tvas.download', ['factureTva' => $this->record->id]))
                ->openUrlInNewTab(true),
            Actions\Action::make('print')
                ->label('Imprimer')
                ->icon('heroicon-o-printer')
                ->url(route('facture-tvas.print', ['factureTva' => $this->record->id]))
                ->openUrlInNewTab(true),
            ActionGroup::make([
