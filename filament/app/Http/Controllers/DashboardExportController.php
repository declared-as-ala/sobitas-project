...
class DashboardExportController extends Controller
{
     ...
     * Returns CSV download with proper headers; on failure returns a minimal error CSV so the user gets a file instead of an HTML error page.
     */
    public function export(Request $request): StreamedResponse
    {
        $preset = $request->get('preset', '30_days');
        $filename = 'dashboard-export-' . Carbon::now()->format('Y-m-d-His') . '.csv';
        ...
        try {
            ...
            return $this->downloadCsvError($filename, $e->getMessage());
        }
    }

    /**
     * Return a minimal CSV with a single error row so the browser receives a file and not an HTML error page.
     */
    private function downloadCsvError(string $filename, string $message): StreamedResponse
    {
        return response()->streamDownload(function () use ($message) {
            $handle = fopen('php://output', 'w');
            fprintf($handle, chr(0xEF) . chr(0xBB) . chr(0xBF));
            fputcsv($handle, ['Erreur', $message]);
            fclose($handle);
        }, $filename, [
