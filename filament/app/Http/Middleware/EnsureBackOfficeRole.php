 ...
 * `auth` answers "is somebody logged in", not "is this person staff" — and affilies log in on the
 * SAME `web` guard as admins (AffiliePanelProvider declares no ->authGuard(); config/auth.php
 * defines only 'web'). The Filament affilie panel itself is correctly isolated, but these routes
 * live OUTSIDE the panel, so panel isolation never applied to them.
 *
 * The result, confirmed against the code before this middleware existed: any affilie could call
 *
 ...
 * facture TVA, devis and wholesale price list by ID — including rival affilies' sales and the
 * affilie codes printed on them — plus the company revenue, stock and loyalty exports.
 *
 * Affilies are competitors of one another. This was the single highest-severity finding in the
 * B2B audit and it had to be closed before public affilie signup could exist at all.
 *
 ...
 * question is the one config/affilies.php already answers for the admin panel, so this reuses
...
class EnsureBackOfficeRole
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $allowed = config('affilies.admin_role_ids', [1, 3]);
