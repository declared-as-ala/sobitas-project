        ...
        <div class="analytics-container">
            <div class="Dashboard Dashboard--full">
                <form role="form" class="form-edit-add"
                    @if (@$edit) action="{{ route('voyager.update_commande', @$facture->id) }}"  @else action="{{ route('voyager.store_commande') }}" @endif
                    method="POST">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="row ">
                            <div class="col-md-6">
                                ...
                                @if (@$facture->adresse1)
                                    <div class="address">{{ @$facture->adresse1 }}</div>
                                @endif
                                @if (@$facture->adresse2)
                                    <div class="address">{{ @$facture->adresse2 }}</div>
                                @endif
                                @if ($facture->ville || $facture->region || $facture->code_postale)
                                    <div class="email"> {{ @$facture->ville }}, {{ @$facture->region }}
                                        {{ @$facture->code_postale }} </div>
                                @endif
                                <div class="email"><b>Email :</b> {{ @$facture->email }}
                                </div>
                                <div class="email"><b>Numéro de téléphone :</b> {{ @$facture->phone }}
                                </div>
                            ...
                            <div class="col-md-6">
                                @if (@$facture->livraison_nom)
                                    ...
                                    <hr style="margin : 9px">
                                    <b class="to"> <b>Nom et prénom:</b> {{ @$facture->livraison_nom }}
                                        {{ @$facture->livraison_prenom }}</b>
                                    @if (@$facture->livraison_adresse1)
                                        <div>{{ @$facture->livraison_adresse1 }}</div>
                                    @endif
                                    @if (@$facture->livraison_adresse2)
                                        <div>{{ @$facture->livraison_adresse2 }}</div>
                                    @endif
                                    @if ($facture->livraison_ville || $facture->livraison_region || $facture->livraison_code_postale)
                                        <div> {{ @$facture->livraison_ville }}, {{ @$facture->livraison_region }}
                                            {{ @$facture->livraison_code_postale }} </div>
                                    @endif
                                    <div><b>Email :</b> {{ @$facture->livraison_email }}
                                    </div>
                                    <div><b>Numéro de téléphone :</b> {{ @$facture->livraison_phone }}
                                    </div>
                                @endif
                            </div>
                        </div>
                                        ...
                                        <tr id="achat{{ $start }}">
                                            ...
                                            <td>
                                                <input type="number" id="qte{{ $start }}"
                                                    ...
                                                    min="1" onkeyup="calculate()" onchange="calculate()">
                                            </td>
                                            <td>
                            ...
                            <div class="col-md-5">
                                <table class="table">
                                    ...
                                    <tr>
                                        ...
                                        <td><input name="frais_livraison" class="form-control" id="frais_livraison"
                                                step='0.001'
                                                @if (@$edit) value="{{ @$facture->frais_livraison }}" @else value="0.000" @endif
                                    ...
                                    <tr id="ligne_apres_remise">
                                        ...
                                        <td><input name="apres_remise" class="form-control" id="apres_remise"
                                                step='0.001'
                                                @if (@$edit) value="{{ @$facture->prix_ht + @$facture->frais_livraison }}" @else value="0.000" @endif
                                                disabled></td>
                                    </tr>
                                    {{--  <tr>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select class="form-control" name="etat">
                                        <option value="nouvelle_commande"
                                            @if (@$facture->etat == 'nouvelle_commande') selected @endif>Nouvelle Commande</option>
                                        <option value="en_cours_de_preparation"
                                            @if (@$facture->etat == 'en_cours_de_preparation') selected @endif>En cours de préparations
                                        </option>
                                        <option value="prete" @if (@$facture->etat == 'prete') selected @endif>Prête
                                        </option>
                                        <option value="en_cours_de_livraison"
                                            @if (@$facture->etat == 'en_cours_de_livraison') selected @endif>En cours de livraison
                                        </option>
                                        <option value="expidee" @if (@$facture->etat == 'expidee') selected @endif>Expidée
                                        </option>
                                        <option value="annuler" @if (@$facture->etat == 'annuler') selected @endif>Annuler
                                        </option>
                                    </select>
            ...
            <table class="table">
                ...
                <tr>
                    ...
                    @foreach ($split as $history)
                        @php
                            $split2 = explode(',', $history);
                            $etat = '';
                            if ($split2[0] == 'nouvelle_commande') {
                                $etat = 'Nouvelle Commande';
                            }
                            if ($split2[0] == 'en_cours_de_preparation') {
                                $etat = 'En cours de préparations';
                            }
                            if ($split2[0] == 'prete') {
                                $etat = 'Prête';
                            }
                            if ($split2[0] == 'en_cours_de_livraison') {
...
@section('javascript')
    ...
    <script>
        ...
        function calculate(type_remise) {
            ...
            var input_apres_remise = document.getElementById('apres_remise')
            input_apres_remise.value = m_totale_ttc;
