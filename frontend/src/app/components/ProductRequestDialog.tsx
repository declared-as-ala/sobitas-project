...
import { ArrowLeft, ArrowRight, Check, Clock3, Loader2, PackageCheck, Truck, X } from 'lucide-react';
    ...
    <SheetContent side="bottom" showCloseButton={false} style={{ backgroundColor: 'rgb(var(--c-elevated))' }} className="max-h-[94dvh] gap-0 overflow-hidden rounded-t-2xl border-hairline bg-elevated sm:inset-y-0 sm:left-auto sm:right-0 sm:h-full sm:max-h-dvh sm:w-full sm:max-w-xl sm:rounded-none">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 sm:p-5" data-request-body>
        {step === 'alternatives' ? <>
          ...
          */}
          {loading ? <div role="status" aria-label="Recherche des alternatives" className="space-y-3"><Skeleton className="h-44 w-full rounded-xl" /><Skeleton className="h-44 w-full rounded-xl" /></div> : rows.length ? <div className="space-y-3" data-request-alternatives>{rows.map((row, index) => <div key={row.id} className={`overflow-hidden rounded-2xl border ${index === 0 ? 'border-brand/40' : 'border-hairline'}`}>
            {index === 0 && <p className="bg-brand/10 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wide text-brand">Le plus proche de votre choix</p>}
            <div className="p-3">
              <div className="flex gap-3">
                <ComparisonProductImage src={row.image} name={row.name} />
                <div className="min-w-0 flex-1">
                  <p className="inline-flex items-center gap-1.5 text-xs font-semibold text-ok"><PackageCheck className="h-3.5 w-3.5 shrink-0" aria-hidden="true" />En stock</p>
                  <LinkWithLoading href={row.url} onClick={() => onOpenChange(false)} className="flex min-h-11 items-center text-sm font-semibold leading-snug text-ink-1 hover:text-brand focus-visible:ring-2 focus-visible:ring-focus">{row.name}</LinkWithLoading>
                  <p className="text-xs text-ink-2">{[row.category, row.format].filter(Boolean).join(' · ')}</p>
                </div>
              </div>
              <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-hairline pt-3 text-xs text-ink-2">
                {row.facts.protein && <span><strong className="text-ink-1">{row.facts.protein}</strong> de protéines</span>}
                <span className="inline-flex items-center gap-1"><Truck className="h-3.5 w-3.5 shrink-0 text-ok" aria-hidden="true" />Livré en 24-72 h</span>
              </div>
              <div className="mt-3 flex items-baseline gap-2">
                <strong className="font-display text-xl text-brand">{formatTnd(row.price)}</strong>
                {row.oldPrice != null && <span className="text-xs text-ink-2 line-through">{formatTnd(row.oldPrice)}</span>}
              </div>
              <LinkWithLoading href={row.url} onClick={() => onOpenChange(false)} className="mt-3 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-brand px-3 font-display text-sm font-bold uppercase tracking-wide text-on-brand transition-colors hover:bg-brand-hover focus-visible:ring-2 focus-visible:ring-focus">Choisir celui-ci<ArrowRight className="h-4 w-4 shrink-0" aria-hidden="true" /></LinkWithLoading>
            </div>
