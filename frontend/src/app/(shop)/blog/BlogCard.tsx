  ...
  return (
    <article className="group h-full min-w-0">
      ...
      >
        ...
        <div className={`flex min-w-0 flex-1 flex-col ${variant === 'compact' ? 'p-3.5 sm:p-4' : 'p-4 sm:p-[1.125rem]'}`}>
          {eyebrow && (
            <span className="mb-1.5 font-display text-[10px] font-semibold uppercase tracking-[0.16em] text-brand">
              {eyebrow}
            </span>
          )}
          <Title
            className={`line-clamp-2 font-display font-bold leading-[1.3] tracking-[-0.015em] text-ink-1 transition-colors group-hover:text-brand ${
              variant === 'feature'
                ? 'mb-2.5 text-xl sm:text-2xl lg:text-[1.625rem]'
                : variant === 'compact'
                  ? 'mb-2 text-[0.9375rem] sm:text-base'
                  : 'mb-2 min-h-[2.6em] text-[0.9375rem] sm:text-[1.0625rem]'
            }`}
          >
            {title}
          </Title>
          {excerpt && variant !== 'compact' && (
            <p className={`mb-3.5 flex-1 text-sm leading-6 text-ink-2 ${variant === 'feature' ? 'line-clamp-3 max-w-3xl sm:text-[15px]' : 'line-clamp-2'}`}>
              {excerpt}
            </p>
          )}
          <div className={`mt-auto flex items-center justify-between gap-3 border-t border-rule ${variant === 'compact' ? 'pt-2.5' : 'pt-3'}`}>
