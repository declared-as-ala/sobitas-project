...
/**
 * The Product `description`: the copy the PAGE shows, before the meta-description template.
 *
 * ── WHAT WAS BEING PUBLISHED ────────────────────────────────────────────────────────────────
 * Both builders started their chain at `seo.description || meta_description`, and that field is
 * not editorial copy — it is generated by `filament/app/Services/Seo/ProductSeoDefaults.php` from
 * a fixed template. Measured live 22/09/2026 (bot and human identical):
 *
 *     /creatine/creatine-monohydrate-300g-ostrovit
 *       "CREATINE MONOHYDRATE - 300G - OSTROVIT — en vente en Tunisie. Livraison 24-72h partout
 *        en Tunisie. Paiement à la livraison. Produit 100% authentique."
 *     /mass-gainers/serious-mass-5-45-kg-optimum-nutrition
 *       "SERIOUS MASS 5,45 KG - OPTIMUM NUTRITION — Mass Gainers en Tunisie. Livraison 24-72h
 *        partout en Tunisie. Paiement à la livraison."
 *
 * Across the catalogue that is ONE sentence with the name and the category substituted in — the
 * exact "many pages whose text differs only by a substituted noun" pattern factualProductDescription
 * above exists to avoid, sitting in the field Google is most likely to quote in a product result.
 * The same page meanwhile renders a real multi-paragraph `description_fr` that the WebPage node
 * already quotes, so the Product node contradicted its own page.
 *
 * ── THE ORDER ───────────────────────────────────────────────────────────────────────────────
 * Visible copy first, the meta template second, the attribute-driven generator last. The meta
 * template is still exactly right for `<meta name="description">` — it is written for a SERP
 * snippet — and nothing here changes that.
 *
 * `description_fr` before `description_cover` because that is the order BOTH views that render the
 * description use (ProductDetailClient.tsx:310 and crawler/CrawlerProductView.tsx:79); reversing
 * them here would have re-created the mismatch on every product that has both.
 *
 * The 40-character floor is the same one the sanitize path applies after cleanSourceText: a
 * two-word `description_cover` is not a description, and falling through to the template beats
 * publishing a stub. `cleanSourceText` runs here too so the source retailer's transcribed notice
 * can never reach the field through the newly-preferred `description_fr`.
 *
 * Shared by BOTH builders on purpose: a PDP and a category page describe the same @id, and they
 * were producing two different texts for it.
 */
function productSchemaDescription(product: Product): string {
  /*
   * The imported copy keeps the source page's own heading stack at the TOP — "Informations sur le
   * produit", then "Aperçu" — as <h2>/<h3> that tag-stripping flattens into the first words of the
   * text. `cleanSourceText` cuts the retailer's notice, which is always the FOOTER of the
   * transcribed page, so it never reaches this; same idea, other end of the field.
   *
   * Measured 22/09/2026 on the 40 audit fixtures: 34 of them (68 of the 80 graphs) opened their
   * Product.description with that stack. Google truncates a product snippet at roughly that point,
   * so ~85% of the imported catalogue shared its first four words — which re-creates, one layer
   * down, the duplication that replacing the formulaic template was meant to remove. Do not
   * reinstate it.
   *
   * Matched on the PLAIN text (the headings are markup in the source, so the match only exists
   * after tag-stripping) and re-trimmed. A product whose whole description IS the heading comes
   * back empty, which is below the 40-character floor below, so it falls through to the meta
   * chain exactly as an empty `description_fr` already did.
   */
  const dropHeadingStack = (text: string): string =>
    text.replace(/^\s*Informations sur le produit\s*(Aperçu)?\s*/i, '').trim();

  /* Backend copy arrives as HTML with entities ("Puissance &amp; Performance"), and JSON-LD text
     is plain text: strip the tags, decode, then run jsonLdText so an entity that decodes INTO an
     angle bracket cannot close the <script> block — then drop the source's heading stack. */
  const plain = (value: string): string =>
    dropHeadingStack(jsonLdText(decodeHtmlEntities(stripHtml(cleanSourceText(value), 500))));

  const visible = plain(product.description_fr || product.description_cover || '');
  if (visible.length >= 40) return visible;

  return plain(
    product.seo?.description || product.seo_description || product.meta_description || product.meta_description_fr || ''
  );
}

/**
 * A GTIN hiding in `code_product`, or null.
 *
 * The legacy catalogue stores the manufacturer barcode in `code_product` (Serious Mass 5,45 kg:
 * `748927023800`, a valid UPC-A) and the builders emitted it as `sku`/`productID` only, so the
 * strongest product-identity signal Google has for merchant-listing and knowledge-panel matching
 * was published in a field that means "the retailer's internal reference". `gtin` was filled only
 * from the dedicated `product.gtin` column, which is empty on those rows.
 *
 * The check digit is what makes this safe to infer: `code_product` also holds internal references
 * of every shape, and a wrong `gtin` is worse than none. A run of 8/12/13/14 digits that satisfies
...
function gtinFromCodeProduct(product: Product): string | null {
  ...
  // GS1 weights run 3,1,3,1… leftwards from the digit next to the check digit, for every length.
  let sum = 0;
 ...
 *
 * The cut lands on a word boundary and says so with an ellipsis. A hard slice was harmless while
 * this only ever truncated the short generated fallback; now that productSchemaDescription feeds
 * it the product's real multi-paragraph copy, nearly every PDP reaches the limit, and "…jusqu'à 5
 * g de créat" is a sentence Google can quote back in a product result.
 */
function stripHtml(html: string, maxLen: number = 500): string {
  ...
  // Only back up to the space when one is close enough to the limit to be a real word boundary.
  return `${(lastSpace > maxLen * 0.8 ? cut.slice(0, lastSpace) : cut).replace(/[\s.,;:]+$/, '')}…`;
}
...
export function buildProductJsonLd(product: Product, canonicalUrl: string): object | null {
  ...
  // Visible copy first, meta template second — see productSchemaDescription.
  const description = productSchemaDescription(product);
...
export function sanitizeBackendProductJsonLd(product: Product, raw: unknown, canonicalUrl: string): object | null {
  ...
  const sanitized: Record<string, unknown> = {
    ...
    // Same chain as buildProductJsonLd — the two builders answer the same product URL and must
    // not describe the same @id differently. See productSchemaDescription.
    description:
      productSchemaDescription(product) ||
  ...
  } else if (!['gtin', 'gtin8', 'gtin12', 'gtin13', 'gtin14'].some((key) => sanitized[key] != null)) {
    // No barcode from the column and none carried by the backend graph: infer it from
    // code_product only when the check digit proves it is one. See gtinFromCodeProduct.
    const inferredGtin = gtinFromCodeProduct(product);
    if (inferredGtin) sanitized.gtin = inferredGtin;
  }
...
export function buildSiteNavigationSchema(baseUrl: string): object {
  ...
  const links: Array<{ name: string; path: string }> = [
    ...
    // Mirrors the footer's sitewide link (FooterClient.tsx) character for character: SiteNavigation
    // markup has to describe the navigation users actually see, and the footer now votes for
    // /mass-gainers over /gainers-proteines.
    { name: 'Mass gainer en Tunisie', path: '/mass-gainers' },
 ...
 *
 * STILL AN IDENTITY CLAIM, NOT A PROFILE. `description` stays out: nothing consumes it, and
 * padding the node makes the blob bigger without making the graph better.
 *
 * `logo` WAS out for the same reason and is now in — reversing the note that stood here, which
 * argued "no Google feature reads Brand.logo, and FeaturedBrands.tsx carries an onError fallback
 * precisely because some admin logos 404". The second half of that is still true and is why the
 * property is emitted ONLY when the admin has actually filed a logo (~8% of brands: the curated
 * ones — Optimum Nutrition, BioTech USA, Dymatize — which are also the ones with search demand
 * and a landing page written for them). What changed is the weight of the first half: this node
 * is the single place every PDP's `brand.@id` resolves to, so it is the one place an image can
 * consolidate for the whole catalogue, and an entity image is read by more than one consumer.
 * A brand with no logo row emits exactly the four properties it did before.
 *
 * THE KNOWN RISK, STATED: nothing here can tell a filed logo from a filed logo that 404s. If a
 * curated brand's logo is broken in the admin, this publishes a broken URL — check the 16 curated
 * brands' `logo` values render before treating this as done.
 */
...
export function buildArticleSchema(article: {
  ...
  const plainDesc = (article.description_fr || article.description || '')
    ...
    .slice(0, 200);
  /*
   * `headline` is the article's title AS SHOWN ON THE PAGE, which is the H1 — not the <title>.
   *
   * The chain started at `seo.title`, the SERP title, so every post that has one published its
   * brand-suffixed variant instead. Measured live 22/09/2026 on /blog/prix-de-la-creatine-en-tunisie:
   *
   *     headline  "Prix créatine Tunisie : comparer formats et marques | Protein.tn"
   *     <h1>      "Prix de la créatine en Tunisie : comment comparer les formats et les marques ?"
   *
   * The suffix is also pure waste against Google's ~110-character headline budget. `designation_fr`
   * is the field ArticleDetailClient renders in the H1 (through the same decodeHtmlEntities), so
   * taking it first makes the markup describe the visible page; seo.title stays as a last resort
   * for a post with no designation. jsonLdText keeps a stray tag out of the JSON-LD block.
   */
  const headline =
    jsonLdText(decodeHtmlEntities(article.designation_fr || article.schema?.headline || article.seo?.title || '')) ||
    'Article';
