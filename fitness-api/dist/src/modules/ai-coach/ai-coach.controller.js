"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AiCoachController = void 0;
const common_1 = require("@nestjs/common");
const ai_coach_service_1 = require("./ai-coach.service");
const chat_message_dto_1 = require("./dto/chat-message.dto");
const auth_guard_1 = require("../../auth/auth.guard");
const current_user_decorator_1 = require("../../auth/current-user.decorator");
const swagger_1 = require("@nestjs/swagger");
let AiCoachController = class AiCoachController {
    aiCoachService;
    constructor(aiCoachService) {
        this.aiCoachService = aiCoachService;
    }
    async getChatHistory(user) {
        return this.aiCoachService.getChatHistory(user.id);
    }
    async sendMessage(user, dto) {
        return this.aiCoachService.handleChatMessage(user.id, dto.message);
    }
};
exports.AiCoachController = AiCoachController;
__decorate([
    (0, common_1.Get)('history'),
    (0, swagger_1.ApiOperation)({ summary: 'Get recent conversation history with the AI Coach' }),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [current_user_decorator_1.UserSession]),
    __metadata("design:returntype", Promise)
], AiCoachController.prototype, "getChatHistory", null);
__decorate([
    (0, common_1.Post)(),
    (0, swagger_1.ApiOperation)({ summary: 'Send a message to the AI Coach and get a personalized, multilanguage fitness response' }),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [current_user_decorator_1.UserSession,
        chat_message_dto_1.ChatMessageDto]),
    __metadata("design:returntype", Promise)
], AiCoachController.prototype, "sendMessage", null);
exports.AiCoachController = AiCoachController = __decorate([
    (0, swagger_1.ApiTags)('AI Coach'),
    (0, swagger_1.ApiBearerAuth)(),
    (0, common_1.UseGuards)(auth_guard_1.AuthGuard),
    (0, common_1.Controller)('api/v1/ai-coach'),
    __metadata("design:paramtypes", [ai_coach_service_1.AiCoachService])
], AiCoachController);
//# sourceMappingURL=ai-coach.controller.js.map